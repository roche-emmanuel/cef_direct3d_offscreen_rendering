...
typedef std::map<void*, EGLSurface> NervSurfaceMap;
NervSurfaceMap gSurfaces;

// Our own context used to do the rendering:
EGLContext nvContext = EGL_NO_CONTEXT;
EGLConfig nvConfig = nullptr;

// Parameters used to create our window:
HWND hwnd = nullptr;
EGLDisplay winDisplay = EGL_NO_DISPLAY;
EGLContext winContext = EGL_NO_CONTEXT;
...
GLuint program = 0;
GLuint nvTexture = 0;
GLuint nvVertBuffer = 0;
GLuint nvIdxBuffer = 0;
GLint posLoc = -1;
GLint coordLoc = -1;
GLint texLoc = -1;


#define NV_CHECK_GL(msg) {\
  GLenum val = glGetError(); \
  if (val != GL_NO_ERROR) \
  {                      \
    NV_LOG(msg << " (error code: "<<val<<")");      \
    return;              \
  } \
}

#define NV_CHECK_GL_RET(msg, ret) {\
  GLenum val = glGetError(); \
  if (val != GL_NO_ERROR) \
  {                      \
    NV_LOG(msg << " (error code: "<<val<<")");      \
...
#define NV_CHECK_RET(cond, ret, msg) \
  if (!(cond))                    \
  {                               \
    NV_LOG(msg);               \
    return ret;                   \
...
GLuint LoadShader(GLenum type, const char *shaderSrc)
{
  ...
  NV_CHECK_RET(shader != 0, 0, "Cannot create shader.");
...
bool nvInitContext() 
{
  ...
  NV_CHECK_RET(programObject != 0, false, "Cannot create program.");
  ...
  // glBindAttribLocation(program, 1, "vCoords");

  // posLoc = glGetAttribLocation(program, "vPosition");
  // NV_CHECK_RET(posLoc >= 0, false, "Invalid posLoc value.")
  // coordLoc = glGetAttribLocation(program, "vCoords");
  // NV_CHECK_RET(coordLoc >= 0, false, "Invalid coordLoc value.")

  // Get the sampler location
  texLoc = glGetUniformLocation(program, "tex0");
  NV_CHECK_RET(texLoc >= 0, false, "Invalid texLoc value.")

  // Generate a handle for the rendered texture.
  glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
  glGenTextures(1, &nvTexture);

  // // Bind and configure the texture parameters.
  ...
  // glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, pWidth, pHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels);

  // Init our vertex buffer:
  NV_CHECK_GL_RET("Init1", false);
  glGenBuffersARB(1, &nvVertBuffer);
  NV_CHECK_GL_RET("Init2", false);
  NV_CHECK_RET(nvVertBuffer > 0, false, "Invalid vertex buffer.")

  GLint curBuffer = 0;
  glGetIntegerv(GL_ARRAY_BUFFER_BINDING, &curBuffer);
  NV_CHECK_GL_RET("Init3", false);

  glBindBuffer(GL_ARRAY_BUFFER, nvVertBuffer);
  NV_CHECK_GL_RET("Init4", false);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*20, vVertices, GL_STATIC_DRAW);
  NV_CHECK_GL_RET("Init5", false);

  // Restore the previous buffer:
  glBindBuffer(GL_ARRAY_BUFFER, curBuffer);
  NV_CHECK_GL_RET("Init6", false);

  // Init our indices buffer:
  glGenBuffersARB(1, &nvIdxBuffer);
  NV_CHECK_GL_RET("Init7", false);
  NV_CHECK_RET(nvIdxBuffer > 0, false, "Invalid index buffer.")

  GLint curIdxBuffer = 0;
  glGetIntegerv(GL_ELEMENT_ARRAY_BUFFER_BINDING, &curIdxBuffer);
  NV_CHECK_GL_RET("Init8", false);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, nvIdxBuffer);
  NV_CHECK_GL_RET("Init9", false);

  GLushort indices[] = {0, 1, 2, 0, 2, 3};

  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLushort)*6, indices, GL_STATIC_DRAW);
  NV_CHECK_GL_RET("Init10", false);

  // Restore the previous buffer:
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, curIdxBuffer);
  NV_CHECK_GL_RET("Init11", false);
...
error::Error GLES2DecoderImpl::HandleNervCopyTextureToSharedHandle(
  ...
  if(egl_display == EGL_NO_DISPLAY) {
    NV_LOG("GLES2DecoderImpl: ERROR: Invalid EGL display.");
    return error::kNoError;
  }
  ...
  if(nvConfig == nullptr) {
    ...
    if(eglChooseConfig(egl_display, pattribList, &nvConfig, 1, &numConfigs) != EGL_TRUE) {
      NV_LOG("GLES2DecoderImpl: ERROR: Cannot create EGL rendering config.");
      return error::kNoError;
    }

    if(numConfigs != 1) {
      NV_LOG("GLES2DecoderImpl: ERROR: Unexpected number of pbuffer configs: "<<numConfigs);
      return error::kNoError;
    }

    if(nvConfig == nullptr) {
      NV_LOG("GLES2DecoderImpl: ERROR: Invalid EGL rendering config.");
      return error::kNoError;
    }
  }
  ...
  if(curContext == EGL_NO_CONTEXT) {
    NV_LOG("GLES2DecoderImpl: ERROR: No current EGL context.");
    return error::kNoError;
  }
  ...
  if(nvContext == EGL_NO_CONTEXT) {
    ...
    if(!decContext) {
      NV_LOG("GLES2DecoderImpl: ERROR: Invalid Decoder context.");
      return error::kNoError;
    }
    // Retrieve the share group:
    gl::GLShareGroup* sgroup = decContext->share_group();
    if(!sgroup) {
      NV_LOG("GLES2DecoderImpl: ERROR: Invalid Decoder share group.");
      return error::kNoError;
    }
    ...
    EGLint context_client_major_version = 3;
    ...
    if (!eglBindAPI(EGL_OPENGL_ES_API)) {
      NV_LOG("GLES2DecoderImpl: eglBindApi failed with error: "<< ui::GetLastEGLErrorString());
      return error::kNoError;
    }
    ...
    if (GLSurfaceEGL::IsEGLContextPrioritySupported()) {
      ...
      // Read the priority of the current context and then apply the same here:
    ...
    // It seems that creating a context from a shared context is not working here.
    // So instead, we will try to use the current context directly:
    // nvContext = curContext;
    // Note that our context here is shared with the current context:
  ...
  if(gSurfaces.count(handle)==0) {
    NV_LOG("GLES2DecoderImpl: ERROR: No valid surface, cannot copy to shared handle.");
    return error::kNoError;
  }

  // Here we should copy on the surface
  // cf. https://www.khronos.org/registry/EGL/sdk/docs/man/html/eglMakeCurrent.xhtml
  
  EGLDisplay display = eglGetCurrentDisplay();
  if(display == EGL_NO_DISPLAY) {
    NV_LOG("GLES2DecoderImpl: ERROR: No current EGL display.");
    return error::kNoError;
  }
  
  // Display a notice if this is not the display we woudl expect to see here:
  if(display != egl_display) {
    NV_LOG("GLES2DecoderImpl: EGL display is not the one we were expecting...");
  }
